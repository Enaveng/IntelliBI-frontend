...
import { getChartByIdUsingGET } from '@/services/IntelliBI/chartController';
import ReactECharts from 'echarts-for-react';
import React, { useEffect, useState } from 'react';

const AddChart: React.FC = () => {
  const [chartVo, setChartVo] = useState<API.ChartDataVo>();
      ...
      <Row gutter={24}>
        <Col span={12}>
          ...
          <Card>
            <div style={{ whiteSpace: 'pre-wrap', overflow: 'auto' }}>
              <p style={{ fontWeight: 'bold', textAlign: 'center' }}>{chartVo?.chartData}</p>
            </div>
        ...
        <Col span={12}>
          ...
          <Card style={{ color: 'black' }}>
            <Col>分析目标：{chartVo?.goal}</Col>
            {/*<Col>图表类型：{chartVo?.chartType}</Col>*/}
            {/*<Col>图表名称：{chartVo?.chartName}</Col>*/}
            {/*<Col>分析目标：{new Date(chartVo?.createTime).toLocaleString('zh-CN')}</Col>*/}
            <Spin spinning={submitting} />
          </Card>
          ...
          <Card>
            <div style={{ whiteSpace: 'pre-wrap', overflow: 'auto' }}>
              <p style={{ fontWeight: 'bold', color: '#0b93a1', textAlign: 'left' }}>
                {chartVo?.genResult ?? <div>请先在左侧进行提交</div>}
              </p>
            </div>
            <Spin spinning={submitting} />
          </Card>
