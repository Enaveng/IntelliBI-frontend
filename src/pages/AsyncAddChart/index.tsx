...
import { asyncGenChartByAiMqUsingPOST } from '@/services/IntelliBI/chartController';
...
const AsyncAddChart: React.FC = () => {
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      const res = await asyncGenChartByAiMqUsingPOST(param, {}, values.file.file.originFileObj);
      if (!res?.data) {
        message.error('分析失败');
      } else {
        message.success('图表分析成功，请稍后到 我的图表 界面查看结果');
        form.resetFields(); //表单自动重置
      }
    } catch (e: any) {
